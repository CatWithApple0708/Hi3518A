...
static inline void HIsi3518EnableIRQ(struct HISI_GPIO * gpio,int mask){
	gpio->GPIO_IE |= mask;
}
static inline void HIsi3518DisableIRQ(struct HISI_GPIO * gpio,int mask){
	gpio->GPIO_IE &= ~mask;
}
...
#define GPIO_REG(num,reg)  (HIGPIO##num->reg)
#define DISPLAYY_GPIO_REG(string,num,reg) debug_printk("GPIO%d%s:%d\n",num,string,ioread32(GPIO_REG(num,reg)))
...
static inline void SetHi3518GpioDataM(struct HISI_GPIO * gpio,int mask){
	gpio->DATA |= mask;
}
static inline void ClearHi3518GpioDataM(struct HISI_GPIO * gpio,int mask){
	gpio->DATA &= ~mask;
}
...
static inline void HIsi3518SetDir(struct HISI_GPIO * gpio,int mask,int dir){
	if(dir == DIR_OUTPUT)
		gpio->GPIO_DIR &= ~mask;
	else
		gpio->GPIO_DIR |= mask;
